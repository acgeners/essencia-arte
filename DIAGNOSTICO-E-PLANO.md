...
| COE-3 | 🔴 | **Passo de embalagem inalcançável, mas precificado e exibido.** Wizard tem 5 passos sem packaging, mas `calculatePrice` soma `packagingFee`, RPC grava `p_packaging_id` e Resumo/Confirmação mostram "Embalagem". | [wizard-shell.tsx:29](src/components/public/wizard/wizard-shell.tsx#L29), [step-review.tsx:55](src/components/public/wizard/step-review.tsx#L55) | ✅ removida do fluxo; passo órfão deletado |
...
| COE-5 | 🟡 | **Taxa de personalização `2.0` hardcoded em 3 lugares** (preço, resumo lateral, label). | [calculate.ts:27](src/lib/pricing/calculate.ts#L27), [order-summary-panel.tsx:89](src/components/public/wizard/order-summary-panel.tsx#L89), [step-customization.tsx](src/components/public/wizard/step-customization.tsx) | ✅ `PERSONALIZATION_FEE` centralizada |
| COE-6 | 🟡 | **Categoria com duas fontes de verdade**: coluna `products.category_id` (usada) + tabela M2M `product_category_links` (semeada, morta). | [expand_relations.sql](supabase/migrations/20260427000003_expand_relations.sql) | ✅ migration dropa a tabela M2M |
...
| EF-2 | 🟡 | **`listUsers()` para achar cliente por email** retorna só a 1ª página (~50) → cliente recorrente vira usuário duplicado. | [confirmacao/actions.ts](src/app/(public)/pedido/confirmacao/actions.ts) | ✅ consulta direta na tabela `customers` |
...
| EF-4 | 🟡 | **Totais da sacola são snapshots congelados** no add-time; se o catálogo mudar de preço, cliente vê um valor e é cobrado outro (server recalcula). | [cart-store.ts](src/stores/cart-store.ts), [cart-drawer.tsx](src/components/public/cart-drawer.tsx) | ⏭️ adiado (baixa prioridade pós-Fase 1) |
| EF-5 | 🟡 | **PIX abandonado segura estoque** (baixado na criação do pedido, sem liberação automática). Pede rotina de cancelar `awaiting_payment` antigos e devolver estoque. | [pedidos/actions.ts](src/app/(admin)/admin/pedidos/actions.ts) | ✅ RPC `cancel_order_restore_stock` no cancelamento (auto-expiry = futuro/pg_cron) |
...
| ID | Sev | Problema | Onde | Status |
|---|---|---|---|---|
| UI-1 | 🔴 | **Busca do header é decorativa** — input sem `value`/`onChange`/submit. | [header.tsx](src/components/public/header.tsx), [busca/page.tsx](src/app/(public)/busca/page.tsx) | ✅ busca real (`/busca` + `searchProducts`) |
| UI-2 | 🔴 | **Acompanhamento de pedido é 100% mock** (`setTimeout` retornando dados fixos). | [order-tracking.tsx](src/components/public/order-tracking.tsx) | ✅ busca real por código (`trackOrder`) |
...
| UI-5 | 🟡 | **Links de footer quebrados** (`/produtos`, `/contato` não existem; `/#como-funciona` sem âncora; redes sociais `href="#"`). | [footer.tsx](src/components/public/footer.tsx) | ✅ links corrigidos |
| UI-6 | 🟡 | **Nav "Mais Vendidos" rotula errado** — dropdown contém "Nossas Coleções"; "Outlet" leva a wizard sem outlet. | [header.tsx](src/components/public/header.tsx) | ✅ "Coleções"; "Outlet" removido |
...
| UI-8 | 🟡 | **Prova social fabricada como fato** ("+500 clientes", depoimentos 5★ "de clientes reais") — risco de credibilidade/legal. | [page.tsx:103](src/app/(public)/page.tsx#L103), [testimonials.tsx](src/components/public/testimonials.tsx) | ✅ depoimentos ilustrativos; stats honestos |
...
### Fase 2 — Coerência de fluxo — ✅ **5/6 CONCLUÍDOS (2026-06-24, branch `fase-2-coerencia`)**
- [x] **12. Resolver a embalagem** — `COE-3` — ✅ **removida do fluxo**: tirada de `calculate.ts`, `order-summary-panel`, `step-review` e do `p_packaging_id` no action; passo órfão `step-packaging.tsx` deletado; `packagingFee` fora de `PriceCalculation`. Colunas DB + `catalog.packagingOptions` ficam dormentes (feature futura possível).
- [x] **13. Centralizar a taxa de personalização** — `COE-5` — ✅ `PERSONALIZATION_FEE` em `constants.ts`, usada em `calculate.ts`, `order-summary-panel` e no label de `step-customization`.
- [ ] **14. Recalcular o preço da sacola no checkout** — `EF-4` — ⏭️ **adiado**: após a Fase 1 o valor cobrado (QR/pedido) já é server-side correto; resta só reconciliar a *exibição* da sacola (mismatch raro se o catálogo mudar no meio da sessão). Baixa prioridade.
- [x] **15. Buscar cliente por email diretamente** — `EF-2` — ✅ consulta direta na tabela `customers` (email UNIQUE), elimina o `listUsers()` paginado.
- [x] **16. Cancelamento devolve estoque** — `EF-5` — ✅ RPC `cancel_order_restore_stock` (migration) + `updateOrderStatus` chama no cancelamento. ⚠️ Auto-expirar `pending_payment` antigos ainda precisa de pg_cron (futuro).
- [x] **17. Decidir o modelo de categoria** — `COE-6` — ✅ migration que dropa `product_category_links` (modelo em uso: `products.category_id`).

### Fase 3 — Confiança da loja — 🟦 **EM ANDAMENTO** (PR A vitrine ✅ · PR B admin pendente)

**PR A — Vitrine (2026-06-24, branch `fase-3-vitrine`):**
- [x] **20. Acompanhamento de pedido real** — `UI-2` — ✅ busca real por código (action `trackOrder`), timeline com status canônicos + tratamento de cancelado; copy "tempo real" removida.
- [x] **21. Busca do header real** — `UI-1` — ✅ form → `/busca?q=`; nova página de resultados + query `searchProducts` (ilike por nome).
- [x] **22. Footer e navegação** — `UI-5`/`UI-6` — ✅ footer com links reais (Produtos→/, Como funciona→âncora, WhatsApp real; removidos `/contato` e Instagram morto); "Mais Vendidos"→"Coleções"; "Outlet" removido.
- [x] **23. Prova social suavizada** — `UI-8` — ✅ depoimentos rotulados como ilustrativos (sem claim "clientes reais"); stats fabricados da home (+500 / 5★) trocados por claims verdadeiros.

**PR B — Admin (pendente):**
- [ ] **18. Dashboard real + Relatórios/Financeiro rotulados como "exemplo"** — `UI-3`.
- [ ] **19. Ligar busca e filtro de pedidos no admin** (form + `searchParams`) — `UI-4`.
- [ ] **24. Botões mortos: `disabled` + "Em breve"** (Configurações/Relatórios/Integrações) — `LAY-3`.
...
| 2026-06-24 | **Fase 2 (5/6)** no branch `fase-2-coerencia`: embalagem removida do fluxo (COE-3); `PERSONALIZATION_FEE` centralizada (COE-5); cliente buscado por email sem `listUsers` (EF-2); cancelamento devolve estoque via RPC (EF-5); migration que dropa a M2M (COE-6). EF-4 adiado. **2 migrations novas a aplicar no Supabase.** `next build` + eslint OK. |
| 2026-06-24 | **Fase 3 — PR A (vitrine)** no branch `fase-3-vitrine`: busca real do header (`/busca` + `searchProducts`); acompanhamento de pedido real por código (`trackOrder`) com timeline canônica; footer/nav corrigidos ("Coleções", Outlet removido, links reais); prova social suavizada. `next build` (19 rotas) + eslint OK. PR B (admin) a seguir. |
