-- 20260427000002_update_order_rpc_customer.sql
...
CREATE OR REPLACE FUNCTION create_order_with_stock_check(
  ...
  p_extras jsonb,
  p_customer_id uuid DEFAULT null -- Novo parâmetro opcional
) RETURNS uuid AS $$
DECLARE
  v_order_id uuid;
  v_order_code text;
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    subtotal, shipping_cost, total, deposit, balance, customer_id
  ) VALUES (
    v_order_code, 'pending_payment', p_customer_name, p_customer_phone, p_customer_email, p_customer_notes,
    p_subtotal, p_shipping_cost, p_total, p_deposit, p_balance, p_customer_id
